...
void sample(TH1D* hdata, TH1D* mc_histo, std::vector<std::pair<std::string, TH1D* >> bg_histos, const PlotType PLOT_TYPE) {
  ...
  double Minhight;//min hight of the tstack
...
if (PLOT_TYPE == Z_MASS_FINE)
{
  ...
  hs->GetYaxis()->SetTitleSize(.7 * hs->GetYaxis()->GetTitleSize());
  ...
  TLegend* tl = new TLegend(0.66, 0.7, 0.945, 0.9);
  tl->AddEntry(hdata, "Data", "lp");
  tl->AddEntry(mc_histoCloned, "#gamma^{*}/Z#rightarrow ee(POWHEG)", "f");
  ...
  TLegend* tl2 = new TLegend(0.80, 0.7, 0.945, 0.81);//created second legend to make stuff fit
  ...
  xaxis->SetTitleSize(4.5 * xaxis->GetTitleSize());
  xaxis->SetTitleOffset(offset * xaxis->GetTitleOffset());
  double range = 1.2 * (hPull->GetBinContent(hPull->GetMaximumBin()) - 1);
